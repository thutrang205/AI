...
if not st.session_state['feedback_submitted']:
    ...
    address = binary_input("Khu vực sinh sống (U: Thành phố; R: Nông thôn)", ["U", "R"])
    ...
    schoolsup = binary_input("Có sự tham gia các lớp học phụ đạo tại trường không?  (schoolsup)", ["yes", "no"])
    famsup = binary_input("Có sự hỗ trợ trong giáo dục gia đình (famsup)", ["yes", "no"])
    paid = binary_input("Có tham gia các lớp học thêm ngoài trường học ?(paid)", ["yes", "no"])
    activities = binary_input("Có tham gia hoạt động ngoại khóa (activities)", ["yes", "no"])
    nursery = binary_input("Đã được học qua lớp mẫu giáo? (nursery)", ["yes", "no"])
    higher = binary_input("Muốn học lên cao hơn không? (higher)", ["yes", "no"])
    ...
    if st.session_state['predicted'] and not st.session_state['feedback_submitted']:
        ...
        user_feedback1 = st.slider("Mức độ phản ánh đúng kết quả thực tế: (Thang điểm: 1 - Rất tệ -> 5 - Rất tốt)", 1, 5)
        user_feedback2 = st.slider("Mức độ tin cậy của bạn với mô hình dự đoán: (Thang điểm: 1 - Rất tệ -> 5 - Rất tốt)", 1, 5)
